...
% Determines gene knockouts that move a cell from shallow to deep
...
% source state: shallow quiescence (QDS of 2, 3) [4:9]
% target state: deep quiescence (QDS of 12, 14) [22:27]
...
% source: shallow quiescence (2-3 QDS)
source_mat = norm_data(:, 4:9)

alpha = 1.1; beta = 0.6;

%% run INIT to obtain metabolic model constrained by transcriptomics

shallow_quiescence_model = run_init2(model, alpha, beta, source_mat, fujimaki_19.Gene, 34);
...
% runs CHRR
[v_ref, flux_all_array, rxn_inactive] = run_chrr2(model, shallow_quiescence_model, num_samples);
...
target_mat = norm_data(:, 22:27)


gene_count_table = array2table([source_mat, target_mat])


gene_count_table.Properties.VariableNames = {'shallow_quiescence_1', 'shallow_quiescence_2', 'shallow_quiescence_3', 'shallow_quiescence_4', 'shallow_quiescence_5', 'shallow_quiescence_6',... 
...
diff_table = rnaseqde(gene_count_table, ["deep_quiescence_1", "deep_quiescence_2", "deep_quiescence_3", "deep_quiescence_4", "deep_quiescence_5", "deep_quiescence_6"],...
...
% set inactive reactions to zero
