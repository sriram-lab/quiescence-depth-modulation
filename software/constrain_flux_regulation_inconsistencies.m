function [fluxstate_gurobi, grate, solverobj, inconsistencies] =  constrain_flux_regulation_inconsistencies(model1,onreactions,offreactions,kappa,rho,epsilon,mode,epsilon2,minfluxflag)
...
offreactions_init = offreactions;  % store the original list of offreactions
epsilon2_init = epsilon2;
...
try
    ...
    inconsistencies.totalon = length(onreactions);
    inconsistencies.totaloff = length(offreactions_init);
    if length(onreactions) > 0
        [ix rxnpos] = ismember(onreactions, model1.rxns);
        inconsistencies.on = zeros(length(fluxstate_gurobi), 1);
        inconsistencies.on(rxnpos) = (fluxstate_gurobi(rxnpos) < epsilon); % these are supposed to be above epsilon but they are below
    else
        inconsistencies.on = NaN;
    end
    
    if length(offreactions_init) > 0
        [ix rxnpos] = ismember(offreactions_init, model1.rxns);
        inconsistencies.off = zeros(length(fluxstate_gurobi), 1);
        inconsistencies.off(rxnpos) = (fluxstate_gurobi(rxnpos) > epsilon2_init); % these are supposed to be above epsilon but they are below
    else
        inconsistencies.off  = NaN;
    end
